...
# Helper function to retrieve information based on disease
def helper(dis):
...
# Dictionary for symptoms mapping
symptoms_dict = {
...
def get_predicted_value(patient_symptoms):
    ...
    if not symptoms_dict:
        print("Error: 'symptoms_dict' is missing or not loaded.")
        return "Unknown Disease"

    try:
        # Create an input vector with the correct size
        ...
        # Ensure it's in the correct format for SVC
        input_vector = np.array([input_vector])  # Reshape for sklearn

        # Predict disease
        prediction_index = svc.predict(input_vector)[0]  # Ensure correct shape

        # Check if prediction exists in diseases_list
        if prediction_index in diseases_list:
            return diseases_list[prediction_index]
        else:
            print(f"Error: Predicted index {prediction_index} not found in diseases_list")
            return "Unknown Disease"
